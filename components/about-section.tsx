...
const ORBIT_NODES: OrbitNode[] = [
  { id: "python", label: "Python", icon: "code", angle: Math.PI * 0.8, orbitRadius: 180, speed: 0.0008 },
  { id: "ai", label: "LLMs & RAG", icon: "brain", angle: Math.PI * 0.2, orbitRadius: 180, speed: 0.0008 },
  { id: "pytorch", label: "PyTorch", icon: "layers", angle: Math.PI * 1.3, orbitRadius: 180, speed: 0.0008 },
  { id: "fastapi", label: "FastAPI", icon: "cloud", angle: Math.PI * 1.7, orbitRadius: 180, speed: 0.0008 },
  { id: "qdrant", label: "Vector DBs", icon: "database", angle: Math.PI * 0.5, orbitRadius: 140, speed: 0.001 },
]
...
export function AboutSection() {
  ...
  const fullText = "I am an AI Engineer with hands-on experience building end-to-end Retrieval-Augmented Generation (RAG) systems using Large Language Models (LLMs). I specialize in designing production-ready, multi-tenant AI platforms with FastAPI, PyTorch, and Qdrant. I am passionate about optimizing model inference efficiency and deploying scalable AI microservices."
    ...
    const drawIcon = (x: number, y: number, iconType: string, size: number) => {
      ...
      switch (iconType) {
        ...
        case "brain":
          ...
          ctx.moveTo(x, y - size / 3.5)
          ctx.arc(x - size / 6, y - size / 6, size / 5, Math.PI * 1.5, Math.PI * 2.5, true)
          ctx.arc(x - size / 6, y + size / 6, size / 5, Math.PI * 1.5, Math.PI * 0.5, true)
          ctx.lineTo(x, y + size / 3.5)
          // Right hemisphere arcs
          ctx.moveTo(x, y - size / 3.5)
          ctx.arc(x + size / 6, y - size / 6, size / 5, Math.PI * 1.5, Math.PI * 0.5, false)
          ctx.arc(x + size / 6, y + size / 6, size / 5, Math.PI * 1.5, Math.PI * 0.5, false)
          ctx.lineTo(x, y + size / 3.5)
          ctx.stroke()
          break
          ...
          <div className="space-y-8">
            ...
            >
              <div className="flex items-center gap-2 px-4 py-3 bg-titanium/80 border-b border-cyan/20">
                ...
                <span className="ml-4 text-xs font-mono text-cyan/70">karthik.bio</span>
              </div>
